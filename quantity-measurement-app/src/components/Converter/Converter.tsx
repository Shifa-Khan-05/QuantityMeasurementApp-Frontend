...
import { convertQuantity, compareQuantity, addQuantity, subtractQuantity, divideQuantity } from '../../services/quantityService';
import { getBackendErrorMessage } from '../../utils/backendStatus';
  ...
  const performOperation = async () => {
    ...
    try {
      if (operation === 'convert') {
        if (fromValue.trim() === '' || isNaN(Number(fromValue))) {
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-white px-8 py-6 border-b border-indigo-100 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Accurate conversions for length, weight, volume, and temperature</p>
          </div>
          <div className="px-3 py-1 bg-amber-100 text-amber-700 text-sm font-semibold rounded-full border border-amber-200">
            Guest Mode
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-8">
            {(['convert', 'compare', 'add', 'subtract', 'divide'] as OperationType[]).map((op) => (
              <button
                ...
                title={`${op.charAt(0).toUpperCase() + op.slice(1)} operation`}
              >
                <span>
              ...
              <div className="flex flex-col md:flex-row items-center gap-4">
                <div className="flex-1 w-full bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <input
                    ...
                    placeholder="Enter value"
                    aria-label={`First unit value in ${thisUnit}`}
                    className="w-full text-2xl font-medium text-slate-900 bg-transparent border-0 border-b-2 border-slate-200 focus:ring-0 focus:border-indigo-600 px-0 py-2 mb-4 transition-colors"
                  />
                  <select 
                    value={thisUnit} 
                ...
                <div className="flex-1 w-full bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <input
                    ...
                    placeholder="Enter value"
                    aria-label={`Second unit value in ${thatUnit}`}
                    className="w-full text-2xl font-medium text-slate-900 bg-transparent border-0 border-b-2 border-slate-200 focus:ring-0 focus:border-indigo-600 px-0 py-2 mb-4 transition-colors"
                  />
                  <select 
                    value={thatUnit} 
